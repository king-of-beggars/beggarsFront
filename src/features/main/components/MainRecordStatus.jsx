...
function MainRecordStatus({ isSaved, budget, spend, ratio }) {
  ...
  return (
    <layout.FlexCenterColumn style={{ gap: '5px' }}>
      ...
      >
        <style.MainRecordDateBox ratio={ratio}>
          {dateString}
        </style.MainRecordDateBox>
        <style.MainRecordDayBox ratio={ratio}>
          {dayString}
        </style.MainRecordDayBox>
      </layout.Flex100>
      ...
      <style.MainRecordCardResultRow ratio={ratio}>
        <div>
          {`${isSaved ? '+' : '-'} ${commaOnThree(Math.abs(budget - spend))}`}원
        </div>
