...
import { mainAPI } from 'common/utils/api';
...
export function MainFetcher({ children }) {
  ...
  const { data, isLoading, isError } = useQuery(
    ...
    mainAPI.getMainData,
    {
      ...
      onRetry: (failureCount, error) => {
        console.log(
          `MainAPI.getMainData::: retrying (${failureCount}):`,
          error
        );
      },
    }
  ...
  if (isLoading) {
    return <>{MainRenderer('loading', mainDummyData)}</>;
  } else if (isError) {
    return <>{MainRenderer('error', mainDummyData)}</>;
  }
  ...
  return <>{React.cloneElement(children, { data, isLoggedIn })}</>;
}
