...
import ProgressBarSemiCircle from 'common/components/chart/ProgressBarSemiCircle';

const getDisplayDate = (dateStr) =>
  dateStr.split('T')[0].slice(2).split('-').join('/');
const getDisplayBoardName = (boardNameStr) =>
  boardNameStr.length > 7 ? boardNameStr.slice(0, 7) + '...' : boardNameStr;

// function BoardCard({id, onClickHandler, isDefault, cardData, writeCheck }) {
const BoardCard = React.forwardRef(
  ({ id, onClickHandler, isDefault, cardData, writeCheck }, ref) => {
      ...
      >
        ...
        >
          {isDefault
            ...
            : `${commaOnThree(
                cardData.cashbookId.cashbookNowValue -
                  cardData.cashbookId.cashbookGoalValue
              )}원 낭비`}
        </style.CardSpendText>
      </style.CardBoxContainer>
    );
