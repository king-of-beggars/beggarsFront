...
import { COMMENT } from 'common/constants/comment';
import { useGlobalVariables } from 'common/components/provider/GlobalVariableProvider';
import BoardDetailInput from '../components/BoardDetailInput';
...
import CashDetailModal from 'features/cashbook/components/CashDetailModal';
...
const changeReceiptNumToStr = (num) => {
  ...
  while (str.length < 8) {
    ...
    const randomChar =
      paddingChars[Math.floor(Math.random() * paddingChars.length)];
    str = randomChar + str;
  }
  ...
  if (response) {
    return (
      <style.BackgroundPageLayout
        ...
        isMobile={isMobile}
        backPngTop={isBoasting ? `url(${bgSky70})` : `url(${bgDarkSky})`}
        backPngTail={
          isBoasting ? `url(${bgMountain70})` : `url(${bgDarkMountain})`
        }
        backPngMiddle={isBoasting ? `url(${bgCloud70})` : `url(${bgDarkCloud})`}
        style={{
          ...
          <layout.MainContent>
            ...
            >
              ...
              >
                <AutoTextSize
                  ...
                  maxFontSizePx={24}
                >
                  {!!response.boardName && response.boardName}
                </AutoTextSize>
              </div>
            </layout.HeaderContent>
            <layout.FlexCenterColumn100
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div>
                    {!!response.userId.userNickname &&
                      `${response.userId.userNickname}의 영수증`}
                  </div>
                </layout.FlexCenterRow100>
                <layout.FlexCenterRow100
                  style={{ justifyContent: 'space-between' }}
                >
                  ...
                  <div>
                    {!!response.boardId &&
                      changeReceiptNumToStr(response.boardId)}
                  </div>
                </layout.FlexCenterRow100>
                <layout.FlexCenterRow100
                  style={{ justifyContent: 'space-between' }}
                >
                  ...
                  <div>
                    {!!response.boardCreatedAt &&
                      displayCreatedAt(response.boardCreatedAt, false)}
                  </div>
                </layout.FlexCenterRow100>
              </style.ReceiptInnerContainer>
              <style.ReceiptInnerContainer
                isBoasting={isBoasting}
                padding={`${16 * widthRatio}px`}
                fontSize={`${20 * widthRatio}px`}
              >
              ...
              >
                ...
                >
                  ...
                  <div>
                    {!!response.boardTypes ? '낭비한 금액' : '절약한 금액'}
                  </div>
                  <div>
                    {!!response.cashbookDetail.cashbookGoalValue &&
                      commaOnThree(
            ...
            >
              <style.ReceiptPost isBoasting={isBoasting} ratio={widthRatio}>
                <style.ReceiptMemoTitle
                  isBoasting={isBoasting}
                  ratio={widthRatio}
                >
                  메모
                </style.ReceiptMemoTitle>
                <style.ReceiptMemoContent>
                  <pre>{!!response.boardText && response.boardText}</pre>
                </style.ReceiptMemoContent>
              </style.ReceiptPost>
            </style.ReceiptPostContainer>
            {/* 댓글 */}
