...
import LabeledInput from 'common/components/input/LabledInput';
import LabeledTextarea from 'common/components/input/LabledTextarea';
...
function WriteReceipt({ setClose, cardId, children }) {
  ...
  const onClickSave = () => {
    const newBoard = { boardName: title, boardText: content };
    ...
    <style.ModalOverlay onClick={setClose}>
      ...
      >
        ...
        <layout.FlexCenterColumn100 style={{ fontFamily: 'DOSGothic' }}>
          ...
          />
          <CashBookBtn
            style={{ fontFamily: 'DOSMyungjo', marginTop: '10px' }}
            onClick={onClickSave}
            name="book"
          >
            {comment}
